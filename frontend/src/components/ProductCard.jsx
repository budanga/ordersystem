    ...
    return (
        <div className={`group product-card relative bg-white dark:bg-slate-800/30 rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 transition-all duration-300 shadow-sm ${!isOut && 'hover:border-primary/50 hover:shadow-xl hover:shadow-primary/5'}`}>
            <div className="relative aspect-square overflow-hidden bg-slate-100 dark:bg-slate-900">
                ...
                {/* Dynamically show a badge if it's new, we'll pretend items created recently are new, 
                    or if out of stock, show a badge. For now just show "Out of Stock" badge if 0 */}
                {isOut && (
                ...
                {!isOut && (
                    <div className="add-to-cart absolute inset-x-4 bottom-4 opacity-0 translate-y-2 transition-all duration-300">
                        <button className="w-full py-3 bg-primary text-white font-bold rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-primary/30 cursor-pointer">
            ...
            <div className="p-5">
                ...
                <div className="flex items-center justify-between">
                    ...
                    {/* Placeholder for ratings since backend doesn't supply it yet */}
